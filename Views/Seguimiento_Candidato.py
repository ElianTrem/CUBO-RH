...
def get_rows_from_database():
    ...
    try:
        ...
        cursor.execute(
            """
                SELECT 
                    c.id AS "ID del candidato",
                    c.nombre AS "Nombre del candidato",
                    p.nombre AS "Puesto al que aplica",
                    t.titulo AS "Título académico",
                    e.estado AS "Estado del candidato"
                FROM 
                    candidatos c
                JOIN 
                    puestos p ON c.puesto_id = p.id
                JOIN 
                    titulos_academicos t ON c.titulo_academico_id = t.id
                JOIN 
...
class Seguimiento(QWidget):
    ...
    def on_label_click(self, id_candidato):
        print(f"ID del candidato: {id_candidato}")
        dialog = AddCandidato(candidato_id=id_candidato)
        dialog.exec_()
